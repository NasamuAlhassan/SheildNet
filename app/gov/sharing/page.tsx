    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-foreground-muted text-sm mt-1">Classified intelligence bulletins from partner agencies</p>
        </div>
          ...
          <div key={b.id} className="card-glow rounded-xl p-5">
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="text-xs px-2 py-0.5 rounded bg-background-alt text-foreground-muted font-mono">{b.from}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full border font-medium ${
                  b.classification === 'CONFIDENTIAL' ? 'bg-red-400/10 text-red-400 border-red-400/20' :
                  'bg-slate-400/10 text-foreground-muted border-slate-400/20'
                }`}>
                  {b.classification}
                </span>
              </div>
            ...
            <h2 className="text-foreground text-sm font-semibold mb-2">{b.title}</h2>
            <p className="text-foreground-muted text-xs leading-relaxed mb-3">{b.summary}</p>
            <p className="text-foreground-muted text-xs">{formatDate(b.date)}</p>
          </div>
        ))}
      </div>
    </div>
