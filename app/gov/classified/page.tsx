...
const FOLDERS = [
  { name: 'UNCLASSIFIED', count: 12, color: 'text-foreground-muted', bg: 'bg-slate-400/10 border-slate-400/20' },
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-foreground-muted text-sm mt-1">Gov Admin access only — role-gated compartments</p>
      </div>
        ...
        {FOLDERS.map((f) => (
          <div key={f.name} className={`card-glow rounded-xl p-6 border ${f.bg} flex flex-col items-center text-center gap-3 cursor-pointer hover:opacity-80 transition-opacity`}>
            ...
            <div>
              ...
              <p className="text-foreground-muted text-xs mt-0.5">{f.count} documents</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="card-glow rounded-xl p-5 flex items-start gap-3 border-amber-400/15">
        ...
        <p className="text-foreground-muted text-xs leading-relaxed">
          <span className="text-foreground font-semibold">Classification Note:</span> All documents stored in this
