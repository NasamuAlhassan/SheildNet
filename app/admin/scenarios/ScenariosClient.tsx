    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-background border border-border rounded-2xl w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="font-grotesk font-bold text-foreground text-sm">{isEdit ? 'Edit Scenario' : 'Add Scenario'}</h2>
          <button onClick={onClose} className="text-foreground-muted hover:text-foreground"><X className="w-4 h-4" /></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <label className="block text-foreground-muted text-xs mb-1.5">Title</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-foreground-muted text-xs mb-1.5">Severity</label>
            ...
            <div>
              <label className="block text-foreground-muted text-xs mb-1.5">Type</label>
          ...
          <div>
            <label className="block text-foreground-muted text-xs mb-1.5">Description</label>
        ...
        <div className="px-5 py-4 border-t border-border flex gap-3">
          <button onClick={onClose} className="flex-1 border border-border text-foreground-muted py-2.5 rounded-xl text-sm">Cancel</button>
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-foreground-muted text-sm mt-1">{scenarios.length} scenarios in library</p>
        </div>
        ...
        {scenarios.map(s => (
          <div key={s.id} className="card-glow rounded-xl px-4 py-3 flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-foreground-secondary text-sm font-medium">{s.title}</p>
              <p className="text-foreground-muted text-xs mt-0.5 line-clamp-1">{s.description}</p>
              <span className="text-xs mt-1 inline-block px-2 py-0.5 bg-background-alt rounded text-foreground-muted capitalize">{s.type?.replace('_', ' ')}</span>
            </div>
            <div className="flex gap-2 flex-shrink-0">
              <button onClick={() => setModal(s)} className="text-xs text-foreground-muted hover:text-foreground border border-border hover:border-border-strong p-1.5 rounded-lg transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
