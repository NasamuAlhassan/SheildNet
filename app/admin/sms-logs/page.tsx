  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">All Moolre SMS dispatches — {logs?.length ?? 0} total</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {(['sent', 'delivered', 'failed'] as const).map((s) => (
          <div key={s} className="card-glow rounded-xl p-4 text-center">
            ...
            <p className="text-slate-500 text-xs mt-1 capitalize">{s}</p>
          </div>
        ))}
      </div>
            ...
            <thead>
              <tr className="border-b border-[#1e293b]">
                {['Recipient', 'Phone', 'Message', 'Moolre Ref', 'Status', 'Sent At'].map(h => (
                  <th key={h} className="text-left text-slate-500 font-normal px-4 py-2.5">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[#1e293b]">
              {(!logs || logs.length === 0) ? (
                <tr><td colSpan={6} className="text-center py-10 text-slate-500">No SMS dispatches yet</td></tr>
              ) : logs.map((log) => (
                <tr key={log.id} className="hover:bg-[#0d1426]/40">
                  <td className="px-4 py-3">
                    <p className="text-slate-200">{(log.user as { name?: string })?.name ?? '—'}</p>
                    <p className="text-slate-500">{(log.user as { email?: string })?.email}</p>
                  </td>
                  <td className="px-4 py-3 font-mono text-slate-300">{log.phone}</td>
                  <td className="px-4 py-3 text-slate-400 max-w-[200px] truncate">{log.message}</td>
                  <td className="px-4 py-3 font-mono text-slate-500 max-w-[120px] truncate">{log.moolre_ref ?? '—'}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-slate-500 whitespace-nowrap">{formatDateTime(log.sent_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
