  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-foreground-muted text-sm mt-1">Real-time endpoint health across your organisation</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Offline', value: MOCK_ENDPOINTS.filter(e => e.health === 'offline').length, color: 'text-foreground-muted' },
      ...
      <div className="card-glow rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-border">
          <h2 className="font-grotesk font-semibold text-foreground text-sm">Monitored Endpoints</h2>
        </div>
          ...
          {MOCK_ENDPOINTS.map((ep) => (
            <div key={ep.hostname} className="px-5 py-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-foreground text-sm font-medium font-mono">{ep.hostname}</p>
                <p className="text-foreground-muted text-xs">{ep.os}</p>
              </div>
              <div className="hidden sm:flex items-center gap-1 text-xs text-foreground-muted">
                <Clock className="w-3 h-3" /> {ep.lastCheckIn}
              </div>
              <div className="hidden md:block text-xs text-foreground-muted">{ep.processes} processes</div>
              {ep.events > 0 ? (
