  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-foreground-muted text-sm mt-1">Employee phishing awareness training campaigns</p>
      </div>
      ...
      <div className="card-glow rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="font-grotesk font-bold text-foreground">Q2 2026 — Microsoft 365 Phishing Sim</h2>
            <p className="text-foreground-muted text-xs mt-0.5">Launched June 1, 2026 &bull; {MOCK_RESULTS.length} participants</p>
          </div>
      ...
      <div className="card-glow rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-border flex items-center justify-between">
          <h2 className="font-grotesk font-semibold text-foreground text-sm">Employee Results</h2>
          ...
          <thead>
            <tr className="border-b border-border">
              {['Employee', 'Clicked Link', 'Reported Phish', 'Training Done'].map((h) => (
                <th key={h} className="text-left text-foreground-muted text-xs font-normal px-5 py-2.5">{h}</th>
              ))}
            </tr>
          </thead>
            ...
            {MOCK_RESULTS.map((r) => (
              <tr key={r.email}>
                <td className="px-5 py-3">
                  <p className="text-foreground-secondary text-sm">{r.name}</p>
                  <p className="text-foreground-muted text-xs">{r.email}</p>
                </td>
                <td className="px-5 py-3">
                ...
                <td className="px-5 py-3">
                  <span className={`text-xs ${r.reported ? 'text-emerald-400' : 'text-foreground-muted'}`}>
                    {r.reported ? 'Yes ✓' : 'No'}
                  </span>
                </td>
                <td className="px-5 py-3">
