  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl">
      <div>
        ...
        <p className="text-foreground-muted text-sm mt-1">{business?.company_name}</p>
      </div>
      ...
      <div className="card-glow rounded-xl p-4 flex items-start gap-3">
        ...
        <p className="text-foreground-muted text-xs leading-relaxed">
          <span className="text-foreground font-semibold">SLA Breach Compensation:</span> Up to 30% monthly fee credit per
      ...
      {/* Plan options */}
      <div>
        <h2 className="font-grotesk text-lg font-bold text-foreground mb-4">Change Plan</h2>
          ...
          {BUSINESS_PLANS.map((plan) => {
            ...
            return (
              <div key={plan.id} className={`card-glow rounded-xl p-5 flex flex-col relative ${plan.highlight ? 'border-blue-500/30' : ''}`}>
                {plan.highlight && (
                  <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-blue-600 text-foreground text-xs font-bold rounded-full">
                    POPULAR
                  </div>
                )}
                <h3 className="font-grotesk font-bold text-foreground text-sm mb-1">{plan.name}</h3>
                <p className="text-foreground-muted text-xs mb-2">{plan.limit}</p>
                <p className="font-grotesk text-xl font-bold text-foreground mb-3">
                  {plan.price}<span className="text-foreground-muted text-xs">/mo</span>
                </p>
                ...
                {isCurrent ? (
                  ...
                  <PaymentButton
                    ...
                    className={`w-full py-2.5 rounded-xl text-xs font-semibold ${plan.highlight ? 'bg-blue-600 hover:bg-blue-500 text-foreground' : 'border border-border hover:border-blue-400/30 text-foreground-secondary'}`}
                  />
                )}
              </div>
            );
