    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-foreground-muted text-sm mt-1">All alerts across your team</p>
        </div>
      </div>

      {/* Trigger alert panel */}
      <div className="card-glow rounded-xl p-5">
        <h2 className="font-grotesk font-semibold text-foreground text-sm mb-3 flex items-center gap-2">
          <Zap className="w-4 h-4 text-amber-400" /> Trigger Alert (Admin)
        </h2>
        <p className="text-foreground-muted text-xs mb-4">Select a threat scenario and trigger it for a team member. Critical alerts auto-send SMS.</p>
        <div className="grid sm:grid-cols-3 gap-3">
          {(allAlerts ?? []).slice(0, 6).map((a) => (
            <div key={a.id} className={`p-3 rounded-xl border cursor-pointer hover:border-blue-400/40 transition-all text-xs ${
              ...
              <p className="text-foreground-secondary mt-0.5 line-clamp-1">{a.title}</p>
            </div>
          ))}
        </div>
        <p className="text-foreground-muted text-xs mt-3">Full trigger UI available in admin panel → Alert Management</p>
      </div>
      ...
      <div className="card-glow rounded-xl overflow-hidden">
        {(!alerts || alerts.length === 0) ? (
          ...
          <div className="divide-y divide-border">
            {alerts.map((ua) => (
              <div key={ua.id} className="p-4 flex items-start gap-4 hover:bg-background/40 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-foreground-secondary text-sm font-medium">{ua.alert?.title}</p>
                  <p className="text-foreground-muted text-xs mt-0.5">{(ua as { user?: { name?: string } }).user?.name ?? 'Team member'} &bull; {formatDateTime(ua.created_at)}</p>
                </div>
